     ...
     )
{
     ...
     if ((inclm < 5.2359877e-2) || (inclm > sgp4_pi - 5.2359877e-2))
       shs = 0.0;
     if (sinim != 0.0)
       shs = shs / sinim;
     sgs  = sghs - cosim * shs;
     ...
     )
{
     ...
     // the old check used 1.0 + cos(sgp4_pi-1.0e-9), but then compared it to
...
double  gstime
   ...
   {
     const double twopi = 2.0 * sgp4_pi;
     const double deg2rad = sgp4_pi / 180.0;
     double       temp, tut1;
